...
/*
	Patched by debauer BEGIN
*/ 

void UTFT::drawBitmap(int x, int y, int sx, int sy, bitmapdatatype data, int deg, int rox, int roy, uint32_t bg)
{
	unsigned int col;
	int tx, ty, newx, newy;
	double radian;
	...
	else
	{
		...
		for (ty=0; ty<sy; ty++)
			for (tx=0; tx<sx; tx++)
			{
				col=pgm_read_word(&data[(ty*sx)+tx]);
				if(col != bg){
					newx=x+rox+(((tx-rox)*cosinus[deg])-((ty-roy)*sinus[deg]));
					newy=y+roy+(((ty-roy)*cosinus[deg])+((tx-rox)*sinus[deg]));
